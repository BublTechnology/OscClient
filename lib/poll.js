  ...
  commandStatus: function (client, commandId, deferred, initialTimeStamp, statusCallback) {
    var intervalId = setInterval(function () {
      client.commandsStatus(commandId)
        .then(function (res) {
          if (!res.body) {
            throw new Error('Incorrectly formatted response from commandsStatus -- response must have a body')
          }
          if (res.body.error) {
          ...
          if (res.body.state !== 'inProgress') {
            deferred.resolve(res.body) // res.response, timeElapsed: (finalTimeStamp - initialTimeStamp)
            clearInterval(intervalId)
          } else if (typeof statusCallback === 'function') {
            statusCallback(res.body) // res.response, res.error
          }
        }, deferred.reject)
    }, this.pollPeriod)
  }
